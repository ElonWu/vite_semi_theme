...
export interface ElonDateRangePickerProps {
  value: Moment[];
...
export const ElonDateRangePicker = ({
  value,
  onChange,
  title,
}: ElonDateRangePickerProps) => {
  const { isMobile } = useGlobal();

  if (isMobile) {
    return <ElonDateRange value={value} onChange={onChange} title={title} />;
  }
